using System.Collections;  // IEnumerator ve koleksiyonlar için gerekli kütüphane
using System.Collections.Generic;  // Generic koleksiyonlar için gerekli kütüphane (List, Dictionary vb.)
...
public class TransportLevel : MonoBehaviour
{
    [SerializeField]
    ...
    private void Start()
    {
        ...
        if (!PlayerPrefs.HasKey("TransportCurrentLevel"))
        {
            PlayerPrefs.SetInt("TransportCurrentLevel", 1);  // Ýlk seviyeyi açýk olarak ayarla
        }

        // Kaç seviyenin kilidinin açýldýðýný oku ve LevelText'i güncelle
        unlockLevelsNumber = PlayerPrefs.GetInt("TransportCurrentLevel");
        LevelText.text = PlayerPrefs.GetInt("TransportCurrentLevel", 0).ToString();
    ...
    private void SetPanelsActiveState()
    {
        ...
        if (unlockLevelsNumber > 1)
        {
            ...
            lvl2.enabled = true;
            PlayerPrefs.SetInt("TransportCurrentLevel", 2);  // Geçerli seviyeyi 1 olarak ayarla
        }
        else
        {
            lvl2.enabled = false;
        ...
        if (unlockLevelsNumber > 2)
        {
            ...
            lvl3.enabled = true;
            PlayerPrefs.SetInt("TransportCurrentLevel", 3);  // Geçerli seviyeyi 2 olarak ayarla
        }
        else
        {
            lvl3.enabled = false;
        ...
        if (unlockLevelsNumber > 3)
        {
            ...
            PlayerPrefs.SetInt("TransportCurrentLevel", 3);  // Geçerli seviyeyi 3 olarak güncelle
        }
